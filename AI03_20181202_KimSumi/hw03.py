...
def logic01_02():
    ...
    RainOrSprink = Or(Rain,Sprinklers)
    return Equiv(Wet,RainOrSprink)
...
def logic02_01():
    ...
    return Forall('$x',Exists('$y',Equiv(Person('$x'),Mother('$x','$y'))))
...
def logic02_03():
    ...
    return Forall('$x', Forall('$y',
                Equiv(Daughter('$y','$x'), And(Child('$y','$x'),Female('$x')))))
...
def logic02_04():
    ...
    # defining Grandmother in terms of Female and Parent.
    return Forall('$x', Forall('$z', Equiv(Grandmother('$z','$x'),
        Exists('$y', And(And(Parent('$z','$y'),Parent('$y','$x')),Female('$x'))))))
    ################################################################
...
SOLVE: 증언 (1)을 제외한 (2),(3),(4),(5),(6)을 구현하시오.
"""
...
def suspect():
    ...
    # (2) Susan: “It was Nicole”
    formulas.append(Equiv(TellTruth(susan), CrashedServer(nicole)))
    # (3) Mark: “No,it was Susan”
    formulas.append(Equiv(TellTruth(mark), CrashedServer(susan)))
    # (4) Nicole: “Susan’s a liar.”
    formulas.append(Equiv(TellTruth(nicole), Not(TellTruth(susan))))
    # (5) Exactly one person is telling the truth.
    formulas.append(Or(
        And(And(Not(TellTruth(john)),Not(TellTruth(susan))),Xor(TellTruth(nicole),TellTruth(mark))),
        And(And(Not(TellTruth(nicole)),Not(TellTruth(mark))),Xor(TellTruth(john),TellTruth(susan)))))
    # (6) Exactly one person crashed the server.
    formulas.append(Or(
        And(And(Not(CrashedServer(john)),Not(CrashedServer(susan))),Xor(CrashedServer(nicole),CrashedServer(mark))),
        And(And(Not(CrashedServer(nicole)),Not(CrashedServer(mark))),Xor(CrashedServer(john),CrashedServer(susan)))))
    # Query: Who did it?
...
def number_theorem():
    ...
    formulas.append(Forall('$x', Exists('$y',
                    And(And(Successor('$x','$y'),Forall('$z', Implies(Successor('$x','$z'), Equals('$y','$z')))),
                    Not(Equals('$x','$y'))))))
